...
class AfterPaymentController extends Controller {
    ...
    public function index(Request $request): View | JsonResponse {
        try {
            if ($request->ajax()) {
                $data = BookingCancellationAfterAppointment::with(['canceledBy', 'requestedBy'])->latest()->get();
    ...
    /**
     * Display the booking cancellation after payment details.
     *
     * @param int $id
     * @return JsonResponse
     */
    public function show(int $id): JsonResponse {
        try {
            $cancelInfo = BookingCancellationAfterAppointment::with(['canceledBy', 'requestedBy', 'booking'])->findOrFail($id);
